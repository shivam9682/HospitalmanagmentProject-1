...
import in.sp.spring.service.adminEmail;
...
public class Adminforgot {
	...
	private String generateOtpEmailTemplate(int otp) {
	 ...
	 @RequestMapping("/adminforgot")
	  public String openEmailFormadmin() {
		 return "admin-forgot--email-form";
    ...
    public String Sendotp(@RequestParam("username") String username, HttpSession session) {
        ...
        String message = "<!DOCTYPE html>" +
            ...
            "            <h1>OTP Verification for Admin</h1>" +
            "        </div>" +
            "        <div class='content'>" +
            "            <p class='message'>" +
        ...
        String to = username;
        boolean flag = this.adminEmail.sendEmail(subject, message, to);
     ...
     @PostMapping("/admin-varify")
      public String varifyotp(@RequestParam("otp") int otp, HttpSession session) {
    	  int myotp = (int)session.getAttribute("otp");
    	   String username  =(String)session.getAttribute("username"); 
    	   if (myotp==otp) {
    		     
    		 Admin user=this.adminRepository.getAdminByUsername(username);
    		   
    		 if (user== null) {
				//sende error massage 
    			  session.setAttribute("massage","User does't exits this email !" );
    			   return "admin-forgot--email-form";
			...
			return "admin_password_change_form";
		}
    	   else {
     ...
     public String resendOtp(HttpSession session) {
         ...
         boolean flag = this.adminEmail.sendEmail(subject, message, username);
